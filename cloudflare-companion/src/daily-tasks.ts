...
export async function dailyTasksApi(request: Request, env: Env, profile: string, config: CompanionConfigRow): Promise<Response> {
 ...
 if (request.method === 'GET') {
  ...
  return json({tasks:(rows.results||[]).map(r=>JSON.parse(r.data_json))});
 }
 ...
 if(body.action==='start') {
  ...
  const task:DailyTask|undefined=row?JSON.parse(row.data_json):undefined;
 ...
 if(body.task.deleted) {
  ...
  if(timer?.task_json&&JSON.parse(timer.task_json).id===body.task.id)return json({error:'task_timer_running'},409);
 }
 ...
 if((row?.revision||0)!==body.task.revision) return json({error:'task_conflict',remote:row?JSON.parse(row.data_json):null},409);
 const task=completeTask({...body.task,revision:body.task.revision+1},row?JSON.parse(row.data_json):undefined,zonedParts(Date.now(),config.timezone).date,body.requestId);
