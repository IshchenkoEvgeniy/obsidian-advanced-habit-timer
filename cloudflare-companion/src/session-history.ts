...
export async function recordSession(db:D1Database,profile:string,timer:TimerRow,elapsed:number,status:string,note=''):Promise<string>{
 ...
 let task:unknown=null;try{task=timer.task_json?JSON.parse(timer.task_json):null;}catch{/* Ignore malformed task_json. */}
...
// Columns of the `sessions` UNION in sessionHistoryApi (session_history rows + legacy companion_events rows).
interface SessionHistoryRow {
 id: string; started_at: number | null; ended_at: number | null; habit_name: string; duration_sec: number;
 status: string; data_json: string | null; result_json: string | null; legacy: number; session_date: string;
}
export async function sessionHistoryApi(request:Request,env:Env,profile:string,timezone='UTC'):Promise<Response>{
 ...
 env.DB.prepare(common+' SELECT * FROM sessions'+where+' ORDER BY ended_at DESC,id DESC LIMIT 30 OFFSET ?').bind(...params,offset).all<SessionHistoryRow>(),
 ...
 const items=(rows.results||[]).map(r=>{const data:unknown=JSON.parse(String(r.data_json));const result:unknown=r.result_json?JSON.parse(String(r.result_json)):null;return {...r,data,result,data_json:undefined,result_json:undefined};});
